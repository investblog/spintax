...
- **Status:** **Live on WordPress.org as of 2026-05-12 / 2026-05-13** — 2.0.0 (rev 3530118), 2.0.1 (same-day hot-fix), 2.0.2 (docs + UX), 2.0.3 (next-day P1 runtime ACF guard + cumulative-failure tracking + walk lock). 2.0.0 shipped the ACF / post-meta bindings feature in five phases per `docs/spec-acf-bindings.md`. 2.0.1 fixed 5 reviewer findings (2 P1, 3 P2) plus a form-field name collision. 2.0.2 documented Action Scheduler dependency + added an admin notice. 2.0.3 closes two follow-up reviewer findings: the applier now re-verifies ACF `target.field_key` via `acf_get_field()` on every apply (was previously trusting save-time validation, which CLI `import` and ACF-deactivation cycles could bypass) and Bulk Apply tracks failures cumulatively across chunks (was previously per-chunk, so a multi-chunk walk with mid-walk failures could still clear the Stale badge). Walk lock added in 2.0.3 prevents concurrent walks racing on the cumulative flag. Engine timeline: 1.0.0 → 1.1.0 → 1.4.0 → 1.5.0 → 2.0.0 → 2.0.1 → 2.0.2 → 2.0.3.
