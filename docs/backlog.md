...
**Status:** ✅ **FIXED in 2.4.0.** `BindingsAjax::test_binding()` now requires `current_user_can( 'read_post', $post_id )` before returning the rendered preview — the source-agnostic gate the entry below argued for, applied at the Test-panel layer so it covers post/ACF/product context in one place and closes the pre-existing Phase-2 leak alongside the new WooCommerce one. The write path is untouched (it never echoes data back and runs in userless contexts); the CLI `test` command is untouched (shell-trusted). Three regression tests: a CAP-only user is refused a draft they cannot read, allowed a published post, and an editor still previews a draft they can edit. Kept as a record of the reasoning — including why the naive "gate build_for_binding on publish" fix was wrong.
...
**Status:** 5 of 6 open -- the Gate D fresh-eyes review of the 3.1.0 engine catch-up found no P1; these are the P2s and notes that were not fixed before the tag. The one finding that was fixed (a budget-truncated child render being cached under its own key) shipped in 3.1.0. Finding 1 was fixed on 2026-09-17 and is on `main` for 3.2.0.

1. ✅ **FIXED 2026-09-17, in both engines** (unreleased here; shipped in `spintax/core` 0.9.0). The fix is the one this entry proposed -- route lengths memoised per name. `names_that_reach_a_cycle()` records one witness edge per name, so `$via` is a functional graph and every route is a tail running into exactly one cycle; `route_lengths()` measures them all in one pass, each walk stopping at the first name already measured, and `cycle_path()` subtracts what it printed. Measured on PHP 8.4, same machine, one cycle of 16,000 names: **6.1 s -> 0.163 s** here, 7.5 s -> 0.18 s in the sibling. Messages are byte-identical (the tail-into-a-cycle counts are pinned by name in the sibling's suite; here the corpus and the 3,000-document differential cover it), and a deliberate off-by-one in the subtraction goes red on both.

   The original finding, kept because the reasoning is what made it findable: *the "(N more)" cap bounds the message, but the walk that computes it is O(route length) per name, so one cycle of N names costs O(N^2): 1000 names is ~1M steps (fine), a 20k-name cycle (~400 KB template) is ~4x10^8. Same in `spintax/core`. Memoising distance-to-cycle per name makes it linear. Trigger: a real template anywhere near that size.* No real template ever hit it -- it was fixed as part of the 0.9.0 catch-up, not because the trigger fired.
