...
* **Fix: no stray space before a closing quote or bracket.** A question quoted in a sentence rendered as `"Is it audited? ",` and `«Как дела? »,`; a period or comma before a closing quote, a bracket or the end of an HTML attribute did the same (`(really? )`, `title="really? "`). The space is gone, and a quote that opens the next phrase still gets its space. **Rendered text changes for these shapes.**
