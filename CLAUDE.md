...
- **`#set` / `#def` variable semantics (SHIPPED in 3.0.0):** `docs/spec-set-def-semantics.md` — reverted 2.2.0's `#set` collapse-once to macro expansion and added `#def` for roll-once. Released across the plugin, both packages and the OpenCart port; spintax.net still pending.
- **Release protocol:** `docs/release-checklist.md`; **backlog:** `docs/backlog.md`
- **Cross-engine parity corpus (`@spintax/core`, the npm/TS port):** `W:\Projects\spintax-js` — `packages/conformance/fixtures/*.json` is the **shared golden corpus**, and `packages/conformance/php` drives it through *this* engine. It is the only thing binding the engines together, and **it runs in CI in both directions** since 2026-07-14 — the `conformance` job here, and `php-parity` in `spintax-js` against both PHP engines. (This line said "runs in no CI" until 2026-07-19, three days after that stopped being true. It is the pre-push checklist below that is authoritative on how to run it locally.)
...
- `#set %var% = value` — local variable, **a macro**: the value is substituted at every `%var%` reference and whatever brackets it holds re-roll each time. Bracket type is irrelevant.
- `#def %var% = value` — local variable, **roll-once**: the value is rendered a single time and the result is held for every reference. Covers enumerations *and* permutations. **The roll runs AFTER the variable context is assembled** (Renderer Stage 5b), not where 2.2.0's collapse-once pass sat, so a `#def` can read globals and runtime vars; a runtime var of the same name still outranks it. Resolved in dependency order, and that order **follows aliases through `#set`/globals/runtime**, not just direct references. `#include` in a `#def` value is a validation error (includes resolve after the value is frozen). **Use `#def` for a count you also print** — under `#set` the printed number and the agreed form come from different picks, and the block is dropped anyway because the count slot still holds spintax when plurals run (the validator reports this). History: `#set` collapsed once between 2.2.0 and 3.0.0; that was reverted because it contradicted the published docs from the day it shipped — see `docs/spec-set-def-semantics.md`.
...
**`#set` only — `#def` is rejected here, deliberately.** The parsed form is a flat `name => value` map with no record of which directive defined a name, so a global `#def` would validate, persist in the textarea, and never reach the renderer: the reference would render unresolved. Supporting it means a schema change plus a decision about which context layer a rolled global lands in and how a same-named template-local definition interacts. Logged in `docs/backlog.md`; the save-time validator refuses `#def` with its own message rather than a generic syntax error.
